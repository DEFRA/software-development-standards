...
A shared library is code that is packaged, versioned and published to a package registry so that other projects can install it as a dependency. This avoids duplicating code across services, keeps shared logic in sync and provides a clear contract through versioning.
...
- **Abstracting sensitive code.** Isolating code that contains sensitive logic into a private library, so the consuming service can remain open source without exposing sensitive code.
