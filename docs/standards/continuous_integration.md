...
Use [GitHub Actions](https://github.com/features/actions/) for CI builds as it is well integrated with GitHub and is free to use for open source GitHub repositories.

### Commit SHA pinning

When using GitHub Actions, you must pin your actions to a specific commit SHA rather than a branch or tag. This ensures that the exact version of the action you have tested and approved is used in your workflow.
...
The long commit SHA must be used as it is guaranteed to be unique.
...
It is expected that teams regularly review the **Security** tab in your GitHub repository. It provides a continuously-updated view of Dependabot alerts, code scanning results, and secret scanning alerts without needing to trigger a build.
...
You must include SonarQube Cloud in your CI so that it flags problems it spots in your code, like duplication or complexity.

You must configure your CI to report unit test coverage to SonarQube Cloud. It will then include your test coverage in its assessment of your code.
